...
const createEventSource = (source) => new eventsource_1.default(source, {
    headers: {
...
const onLog = (ctx, subject, logLevel, callback, senders) => {
    const source = `${env_1.endpoint('colossus')}/${ctx.account}/${ctx.workspace}/logs?level=${logLevel}`;
...
exports.onEvent = (ctx, sender, subject, keys, callback) => {
    const source = `${env_1.endpoint('colossus')}/${ctx.account}/${ctx.workspace}/events?sender=${sender}${parseKeyToQueryParameter(keys)}`;
    const es = createEventSource(source);
...
exports.onAuth = (account, workspace, state, returnUrl) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        es.onerror = event => {
            es.close();
            reject(new errors_1.SSEConnectionError(`Connection to login server has failed with status ${event.status}`, event.status));
        };
    });
};
