...
const generateListOfRanges = (indexLength) => ramda_1.map((n) => [n * utils_2.MAX_ENTRIES_PER_REQUEST, Math.min((n + 1) * utils_2.MAX_ENTRIES_PER_REQUEST, indexLength)], ramda_1.range(0, Math.ceil(indexLength / utils_2.MAX_ENTRIES_PER_REQUEST)));
const handleExport = (csvPath) => __awaiter(void 0, void 0, void 0, function* () {
    const rawRoutesIndexFiles = yield clients_1.rewriter.routesIndexFiles();
    ...
    const indexHash = yield crypto_1.createHash('md5')
    ...
    const metainfo = yield fs_extra_2.readJson(utils_2.METAINFO_FILE).catch(() => ({}));
    ...
    yield Promise.each(listOfRanges.splice(counter), ([from, to]) => __awaiter(void 0, void 0, void 0, function* () {
        let result;
        try {
            result = yield clients_1.rewriter.exportRedirects(from, to);
        }
        catch (e) {
            yield utils_2.saveMetainfo(metainfo, EXPORTS, indexHash, counter, listOfRoutes);
            listener.close();
            throw e;
        }
    ...
    yield fs_extra_1.writeFile(`./${csvPath}`, csv);
    logger_1.default.info('Finished!\n');
    listener.close();
    utils_2.deleteMetainfo(metainfo, EXPORTS, indexHash);
});
let retryCount = 0;
exports.default = (csvPath) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        yield handleExport(csvPath);
    }
    catch (e) {
        ...
        yield utils_2.sleep(utils_2.RETRY_INTERVAL_S * 1000);
        retryCount++;
        yield module.exports.default(csvPath);
    }
});
