...
class Button extends Component {
  ...
  render() {
    ...
    let classes = `vtex-button bw1 ba fw5 v-mid relative pa0 lh-solid ${className} `
    let labelClasses = `flex h-100 ${classNameFragment} `
    let loaderSize = 15
    ...
    switch (size) {
      case 'small':
        ...
        // horizontalPadding = iconOnly ? 3 : 5 backup
        horizontalPadding = 0
        break
      case 'large':
    ...
    const primaryEnabledClasses =
      'bg-action-primary b--action-primary c-on-action-primary '
    const secondaryEnabledClasses =
      'bg-action-secondary b--action-secondary c-on-action-secondary '
      ...
      switch (variation) {
        ...
        case 'tertiary': {
          if (disabled) {
            ...
            classes += 'bg-transparent b--transparent c-action-primary '
          }
          if (!disabled) {
            classes += ' '
          }
...
Button.propTypes = {
  ...
  classNameFragment: PropTypes.string,
  /** Button size  */
