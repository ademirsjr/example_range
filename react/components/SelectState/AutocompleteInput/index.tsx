...
const CSS_HANDLES = [ 'modalSetStateList' ];

const AutocompleteInput: React.FunctionComponent<PropTypes.InferProps<typeof propTypes>> = ({
	className,
	input: { value, onClear, onSearch, onChange, ...inputProps },
	options: { onSelect, value: options, renderOption, loading, lastSearched = {}, icon }
}) => {
	...
	const showLastSearched = !searching && lastSearched.value && lastSearched.value.length > 0;
	...
	const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
		if (e.key === 'Enter') {
			const selectedOption = selectedOptionIndex !== -1 ? showedOptions[selectedOptionIndex] : term;
	...
	const renderOptions = (): React.ReactElement | React.ReactElement[] => (
		<div className="flex flex-column">
			{showLastSearched ? <div className="pa4 b f6">{lastSearched.label || 'Last searched terms'}</div> : null}
			{showedOptions.map(
				renderOption
					? (option, index) => renderOption(getOptionProps(option, index), index)
	...
	const style = isMobile ? 'br--bottom br2 bb bl br bw1 b--muted-3 shadow-5 bg-base' : 'br bl bb b--silver bg-white';
	...
	return (
		<div ref={containerRef} className="flex flex-column w-100">
			<SearchInput
				{...inputProps}
				className={className}
				value={selectedOptionIndex === -1 ? term : getTermFromOption(showedOptions[selectedOptionIndex])}
			...
			{popoverOpened ? (
				<div className="static">
					<div className={`${handles.modalSetStateList} ${style} absolute w-100 z-5`}>
						{renderOptions()}
						{loading && (
